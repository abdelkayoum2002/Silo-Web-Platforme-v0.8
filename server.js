...
const { disconnectMQTTDevice, attachBroker } = require('./broker');
...
const io = new Server(server, {
  path: "/socket.io"   // 👈 custom namespace path
});
...
app.get('/device/disconnect/me', authenticate, (req, res) => {
  const device_id = req.user.device_id;  
  const user_id = req.user_id;

  if (!device_id) {
    return res.status(400).json({ success: false, error: "No device_id found in user session" });
  }
  ...
  try {
    ...
    if (["Offline", "Disconnected"].includes(ownership.status)) {
      return res.status(400).json({ success: false, error: `Device already ${ownership.status}` });
    }

    // 3. Try disconnect MQTT
    const row = userDb.prepare('SELECT client_id FROM MQTT WHERE device_id = ?').get(device_id);
    if (row?.client_id) {
      disconnectMQTTDevice(row.client_id);
    }

    // 4. Try disconnect Socket.IO
    disconnectSocketDevice(device_id);
    ...
    console.error("❌ Error in /device/disconnect/me:", err);
